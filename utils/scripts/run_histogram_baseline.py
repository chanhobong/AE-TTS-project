#!/usr/bin/env python3
"""
run_histogram_baseline.py

Baseline: patient-level 64-d histogram of prototype assignments.
Train a simple logistic regression classifier and report AUC/PR.
"""
...
def load_patient_histograms(
    ...
    if not X:
        return np.zeros((0, k_clusters), dtype=np.float32), np.zeros((0,), dtype=np.int64), []
...
def save_hist_csv(out_path: Path, X: np.ndarray, y: np.ndarray, ids: List[str]) -> None:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    header = ["patient_id", "label"] + [f"p{i:02d}" for i in range(X.shape[1])]
...
def plot_mean_hist(X: np.ndarray, y: np.ndarray, out_path: Path) -> None:
    ...
    ax.plot(mean0, label="Normal (0)")
    ax.plot(mean1, label="TTS (1)")
    ax.set_title("Mean Prototype Histogram (Normalized)")
    ax.set_xlabel("Prototype ID")
    ax.set_ylabel("Slice Ratio")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Histogram baseline (prototype ratios)")
    ...
    X_train, y_train, train_resolved = load_patient_histograms(
        Path(args.npz_dir), train_ids, labels, args.k_clusters
    )
    X_val, y_val, val_resolved = load_patient_histograms(
        Path(args.npz_dir), val_ids, labels, args.k_clusters
    )
    X_test = np.zeros((0, args.k_clusters), dtype=np.float32)
    y_test = np.zeros((0,), dtype=np.int64)
    test_resolved: List[str] = []
    if args.test_csv:
        X_test, y_test, test_resolved = load_patient_histograms(
    ...
    if args.test_csv and len(X_test) > 0:
        ...
        save_hist_csv(out_dir / "test_histograms.csv", X_test, y_test, test_resolved)

    # Save outputs
    save_hist_csv(out_dir / "train_histograms.csv", X_train, y_train, train_resolved)
    save_hist_csv(out_dir / "val_histograms.csv", X_val, y_val, val_resolved)
    plot_mean_hist(
        np.concatenate([X_train, X_val], axis=0),
        np.concatenate([y_train, y_val], axis=0),
        out_dir / "mean_histogram.png",
    )
    ...
    with open(out_dir / "REPORT.md", "w") as f:
        f.write("# Histogram Baseline (Prototype Ratios)\\n\\n")
        ...
        f.write("\\nOutputs:\\n")
        f.write("- train_histograms.csv\\n")
        f.write("- val_histograms.csv\\n")
        if test_metrics:
            f.write("- test_histograms.csv\\n")
        f.write("- mean_histogram.png\\n")
