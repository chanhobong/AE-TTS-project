#!/usr/bin/env python3
"""
3-panel figure: PCA-2D of patient-level features — mean pooling | std | P90-P10.

Uses trajectory-ordered slice stacks (same as trajectory_volatility_classifier_cv / head sweep).
Colors: Normal (label=0) vs TTS (label=1). Axis labels include each PC's explained variance ratio
(of standardized features, train-fit scaler/PCA).

Example:
  python3 utils/scripts/plot_pooling_pca_triptych.py \\
    ...
    --out_png Report/Draft/fig_pooling_pca_triptych.png
"""
...
def _collect(
    ...
    for p in paths:
        ...
        mean_v = emb.mean(axis=0).astype(np.float64)
        std_v = bl["std"].astype(np.float64)
        p90_v = bl["p90_p10"].astype(np.float64)
        if np.any(~np.isfinite(mean_v)) or np.any(~np.isfinite(std_v)) or np.any(~np.isfinite(p90_v)):
            continue
        xs_m.append(mean_v)
        xs_s.append(std_v)
        xs_p.append(p90_v)
    ...
    if len(pids) < 5:
        raise RuntimeError(f"Too few patients after filters: {len(pids)}")

    return (
        np.stack(xs_m, axis=0),
        np.stack(xs_s, axis=0),
        np.stack(xs_p, axis=0),
...
def _fit_transform_2d(
    ...
    """Returns (Z n×2, explained_variance_ratio length 2) from train-fit StandardScaler + PCA."""
    sc = StandardScaler()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Mean / std / P90-P10 PCA triptych (Normal vs TTS).")
    ap.add_argument("--npz_dir", required=True)
    ap.add_argument("--labels_csv", action="append", required=True)
    ap.add_argument("--train_csv", default=None, help="CSV whose patient_id rows define PCA/scaler fit (default: first --labels_csv).")
    ...
    Zp, evp = _fit_transform_2d(Xp, train_mask)
    ...
    panels = [
        ...
        (Zp, evp, "P90–P10 pooling\n(Robust slice-wise latent variability)"),
    ]
