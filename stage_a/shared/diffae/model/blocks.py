...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1) # -1 남은 차원으로 자동으로 계산해줌 즉 = spatial 임
        qkv = self.qkv(x)
...
# 1. Feature map: (B, C, H, W) → reshape → (B, C, T)
# 2. qkv conv: C → 3C → (B, 3C, T)
# 3. split: Q, K, V 각각 (B, C, T)
# 4. scale: q*scale, k*scale (scale = 1/√√ch = 1/ch^(1/4))
# 5. weight 계산: einsum "bct,bcs→bts" = Q^T·K → (B, T, T)
# 6. softmax: weight = softmax(weight, dim=-1) → (B, T, T)
# 7. value 가중합: einsum "bts,bcs→bct" = weight·V → (B, C, T)
