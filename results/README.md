...
Source: [`main_summary_methods.csv`](main_summary_methods.csv) (copied from `ensemble_fix_mytag_clinical_2/run_mytag`).
...
Ensemble pairs OOS probabilities on matched `repeat_id` + `patient_id` (2900 rows = 100 × 29 test patients).
...
| `towers/*.csv` | Single-tower summaries |
